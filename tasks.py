...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_FUNCTION_CODES_xxx
namespace = Collection("nautobot_function_codes")
namespace.configure(
    {
        "nautobot_function_codes": {
            "nautobot_ver": "3.1.0",
            "project_name": "nautobot-function-codes",
            "python_ver": "3.12",
            "local": False,
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_function_codes.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_function_codes.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_function_codes.nautobot_ver,
        "PYTHON_VER": context.nautobot_function_codes.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_function_codes.project_name}",
        f'--project-directory "{context.nautobot_function_codes.compose_dir}"',
    ]

    for compose_file in context.nautobot_function_codes.compose_files:
        compose_file_path = os.path.join(context.nautobot_function_codes.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_function_codes.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_function_codes.python_ver}...")
    docker_compose(context, command)


def _ensure_creds_env_file(context):
    """Ensure that the development/creds.env file exists."""
    if not os.path.exists(os.path.join(context.nautobot_function_codes.compose_dir, "creds.env")):
        ...
        shutil.copy(
            os.path.join(context.nautobot_function_codes.compose_dir, "creds.example.env"),
            os.path.join(context.nautobot_function_codes.compose_dir, "creds.env"),
        )
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_function_codes.python_ver
    dockerfile_path = os.path.join(context.nautobot_function_codes.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=""):
    ...
    if constrain_nautobot_ver:
        ...
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_function_codes.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {constrain_python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_function_codes"
...
@task(
    help={
        "version": "Version of Nautobot Function Codes to generate the release notes for.",
...
def pylint(context, target=None, recursive=False):
    ...
    if recursive:
        command += " --recursive=y"
    command += f" {' '.join(target) if target else 'nautobot_function_codes'}"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    app_dir = Path(__file__).absolute().parent / Path("nautobot_function_codes")
    migrations_dir = app_dir / Path("migrations")
    migrations_target_module = "nautobot_function_codes.migrations"
    ...
    if migrations_dir.is_dir():
        if run_migrations_check:
            migrations_pylint_command = (
                ...
                " nautobot_function_codes.migrations"
            )
...
def djhtml(context, check=False):
    ...
    command = "djhtml -t 4 nautobot_function_codes/templates/"
...
def generate_test_data(context, flush=False, database=None):
    """Generate test data in Nautobot for Nautobot Function Codes."""
    command = "nautobot-server generate_nautobot_function_codes_test_data"
    if database:
...
def tests(context, failfast=False, keepdb=False, no_input=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_function_codes.local):
